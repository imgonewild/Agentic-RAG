...
def main():
    ...
    # input_csv = input("Enter the name of the input CSV file (e.g., input.csv): ")
    input_csv = 'question.csv'
    output_csv = input("Enter the name for the output CSV file (e.g., output): ") + '.csv'

    # Ensure the paths are in the same directory as the script
    script_dir = os.path.dirname(os.path.abspath(__file__))
    input_csv_path = os.path.join(script_dir, input_csv)
    output_csv_path = os.path.join(script_dir, "output/" + output_csv)

    process_questions(input_csv_path, output_csv_path, document_name)
    
    end_time = time.time()
    response_time = end_time - start_time
    print(f"Response Time: {response_time} seconds")

def process_questions(input_csv, output_csv, document_name):
        ...
        for row in reader:
            if row:  # Check if the row is not empty
                ...
                try:
                    ...
                    source = response_dict.get('source', 'N/A')
